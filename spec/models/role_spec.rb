...
describe Role do
	...
	describe "Ability" do
		...
		context	"set read only ability" do
			before :all do
		...
		context	"set read tenders and create comments" do
			before :all do
			...
			it "can read tenders" do
				expect(@ability.can?(:read, Tender)).to be_truthy
			end

			it "can't create tenders" do
				expect(@ability.can?(:create, Tender)).to be_falsy
			end

			it "can't read comments" do
